        ...
        filteredGroups() {
            ...
            if (Groups != []) {
                for (var i = 0; i < this.groupsList.length; i++) {
                    if (this.groupsList[i].name.includes(this.search) || this.search == "") {
                        Groups.push(this.groupsList[i])
                    }
                }
            }
            return Groups
...
<template>
    <div class="flex flex-col items-center justify-center  bg-gray-100">
        <LargeButton class="m-4" @click="$router.push('/NewGroup')" buttonText=" Add new group">
